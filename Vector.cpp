...
Vector::Vector(const ValueType* rawArray, const size_t size, float coef)
{
    if (coef <= 1.0f)
    {
        throw std::invalid_argument("coef <= 1");
    }
    if (size > 0 && rawArray == nullptr)
    {
        throw std::invalid_argument("size or rawSArray error");
    }
    _size = size;
    _capacity = _size;
...
void Vector::reserve(size_t capacity)
{
    if (capacity < _size)
    {
        throw std::invalid_argument("capacity cant be < size");
    }
...
//изменение памяти до текущего размера
